...
const EMPTY_FORM = {
  unvan: "", vergi_no: "", vergi_dairesi: "", telefon: "", email: "",
...
export default function MusterilerPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const r = await fetch(`/api/musteriler?${params}`);
  ...
  function openEdit(row: any) {
    setEditing(row);
    setForm({
      unvan: row.unvan || "", vergi_no: row.vergi_no || "", vergi_dairesi: row.vergi_dairesi || "",
  ...
  async function save() {
    ...
    try {
      const url = editing ? `/api/musteriler/${editing.id}` : "/api/musteriler";
  ...
  async function deactivate(row: any) {
    ...
    const res = await fetch(`/api/musteriler/${row.id}`, { method: "DELETE" });
    const d = await res.json();
  ...
  const canCreate = hasPermission(user, "musteriler:create");
        ...
        <div className="max-w-3xl mx-auto px-4 py-6">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-zinc-500 text-sm mt-0.5">{rows.length} müşteri — satış faturası carisi</p>
            </div>
            {canCreate && (
          ...
          {loading ? (
            ...
            <div className="text-center py-16 text-zinc-600">Henüz müşteri yok</div>
          ) : (
            <div className="space-y-2">
              {rows.map(row => (
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-md bg-zinc-900 border-l border-zinc-800 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800 shrink-0">
              <h2 className="text-white font-semibold">{editing ? "Müşteriyi Düzenle" : "Yeni Müşteri"}</h2>
