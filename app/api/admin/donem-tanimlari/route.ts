...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!hasPermission(user, "donem_tanimlari:read"))
    ...
    const rows = await getDb().prepare(
      `SELECT * FROM donem_tanimlari ${where} ORDER BY baslangic_tarihi DESC`
    ).all(...params);
...
export async function POST(req: NextRequest) {
  try {
    const user = await requireUser();
    if (!user) return NextResponse.json({ ok: false, error: "Yetkisiz" }, { status: 401 });
    if (!hasPermission(user, "donem_tanimlari:create"))
      return NextResponse.json({ ok: false, error: "Yetersiz yetki" }, { status: 403 });
    ...
    const now = nowIso();
    await db.prepare(
      `INSERT INTO donem_tanimlari (id, donem_adi, baslangic_tarihi, bitis_tarihi, is_active, created_by, created_at, updated_at)
       VALUES (?,?,?,?,?,?,?,?)`
    ).run(id, body.donem_adi.trim(), body.baslangic_tarihi, body.bitis_tarihi, 1, user.id, now, now);
