...
export async function POST(req: NextRequest) {
  try {
    ...
    // columns: [0]=Firma Adı, [1]=Plaka, [2]=Şöför (opsiyonel)
    ...
    for (const r of dataRows) {
      ...
      const result = await db.prepare(
        "INSERT IGNORE INTO company_vehicles (id, company_id, plate, driver_name, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, 1, ?, ?)"
      ).run(uuidv4(), company.id, plate, driverName, now, now);
