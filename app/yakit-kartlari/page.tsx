"use client";
import { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import Link from "next/link";
import Nav from "@/components/Nav";
import ComboboxSearch from "@/components/ComboboxSearch";
import { toast } from "@/lib/toast";
import { hasPermission } from "@/lib/permissions";

const FIRMA_OPTS = ["OPET", "BP", "Shell", "Total", "Petrol_Ofisi", "Diger"];
const FIRMA_LABELS: Record<string, string> = { OPET: "Opet", BP: "BP", Shell: "Shell", Total: "Total", Petrol_Ofisi: "Petrol Ofisi", Diger: "Diğer" };

const EMPTY_FORM = {
  kart_no: "", firma: "OPET", vehicle_id: "", isleten_id: "",
  limit_turu: "sinirsiz", limit_degeri: "", kart_tipi: "normal", notlar: "",
};

export default function YakitKartlariPage() {
  const router = useRouter();
  const [user, setUser] = useState<any>(null);
  const [rows, setRows] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [vehicles, setVehicles] = useState<any[]>([]);
  const [isletenler, setIsletenler] = useState<any[]>([]);
  const [durumFilter, setDurumFilter] = useState("");

  const [showForm, setShowForm] = useState(false);
  const [editing, setEditing] = useState<any>(null);
  const [form, setForm] = useState({ ...EMPTY_FORM });
  const [saving, setSaving] = useState(false);
  const [saveError, setSaveError] = useState<string | null>(null);

  useEffect(() => {
    fetch("/api/auth/me").then(r => r.json()).then(d => {
      if (d.ok) setUser(d.data);
      else router.replace("/login");
    });
    fetch("/api/vehicles?limit=500").then(r => r.json()).then(d => { if (d.ok) setVehicles(d.data); });
    fetch("/api/isletenler?limit=500").then(r => r.json()).then(d => { if (d.ok) setIsletenler(d.data); });
  }, []);

  useEffect(() => { load(); }, [durumFilter]);

  async function load() {
    setLoading(true);
    try {
      const params = new URLSearchParams();
      if (durumFilter) params.set("is_active", durumFilter);
      const r = await fetch(`/api/yakit-kartlari?${params}`);
      const d = await r.json();
      if (d.ok) setRows(d.data);
    } finally { setLoading(false); }
  }

  function openCreate() {
    setEditing(null);
    setForm({ ...EMPTY_FORM });
    setSaveError(null);
    setShowForm(true);
  }

  function openEdit(row: any) {
    setEditing(row);
    setForm({
      kart_no: row.kart_no || "", firma: row.firma || "OPET", vehicle_id: row.vehicle_id || "",
      isleten_id: row.isleten_id || "", limit_turu: row.limit_turu || "sinirsiz",
      limit_degeri: row.limit_degeri != null ? String(row.limit_degeri) : "", kart_tipi: row.kart_tipi || "normal",
      notlar: row.notlar || "",
    });
    setSaveError(null);
    setShowForm(true);
  }

  async function save() {
    if (!form.kart_no.trim()) { setSaveError("Kart no zorunludur"); return; }
    setSaving(true); setSaveError(null);
    try {
      const url = editing ? `/api/yakit-kartlari/${editing.id}` : "/api/yakit-kartlari";
      const method = editing ? "PUT" : "POST";
      const body = { ...form, limit_degeri: form.limit_degeri ? Number(form.limit_degeri) : null };
      const res = await fetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      const d = await res.json();
      if (!d.ok) { setSaveError(d.error || "Kayıt hatası"); return; }
      toast.success(editing ? "Kart güncellendi" : "Kart eklendi");
      setShowForm(false);
      load();
    } finally { setSaving(false); }
  }

  async function toggleActive(row: any) {
    const res = await fetch(`/api/yakit-kartlari/${row.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "durum-degistir", is_active: !row.is_active }),
    });
    const d = await res.json();
    if (!d.ok) { toast.error(d.error || "Hata"); return; }
    toast.success(row.is_active ? "Kart pasife alındı" : "Kart aktifleştirildi");
    load();
  }

  const canCreate = hasPermission(user, "fuel_cards:create");

  return (
    <>
      <Nav user={user} />
      <div className="min-h-screen bg-zinc-950 pt-16">
        <div className="max-w-4xl mx-auto px-4 py-6">

          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-2xl font-bold text-white">Yakıt Kartları</h1>
              <p className="text-zinc-500 text-sm mt-0.5">{rows.length} kart</p>
            </div>
            {canCreate && (
              <button onClick={openCreate}
                className="bg-white text-zinc-950 font-semibold text-sm px-4 py-2 rounded-xl hover:bg-zinc-200 transition-colors whitespace-nowrap">
                + Yeni Kart
              </button>
            )}
          </div>

          <div className="flex gap-2 mb-6">
            {[["", "Tümü"], ["1", "Aktif"], ["0", "Pasif"]].map(([v, l]) => (
              <button key={v} onClick={() => setDurumFilter(v)}
                className={`text-xs px-3 py-1.5 rounded-lg border transition-colors ${
                  durumFilter === v ? "bg-zinc-700 border-zinc-600 text-white" : "border-zinc-800 text-zinc-500 hover:text-white hover:border-zinc-600"
                }`}>
                {l}
              </button>
            ))}
          </div>

          {loading ? (
            <div className="space-y-2">
              {Array.from({ length: 4 }).map((_, i) => <div key={i} className="bg-zinc-900 border border-zinc-800 rounded-xl h-16 animate-pulse" />)}
            </div>
          ) : rows.length === 0 ? (
            <div className="text-center py-16 text-zinc-600">
              <p>Henüz yakıt kartı kaydı yok</p>
              <p className="text-xs mt-1">Araç/işleten bazlı akaryakıt kartlarını ve dolumlarını burada takip edebilirsiniz</p>
            </div>
          ) : (
            <div className="space-y-2">
              {rows.map(row => (
                <div key={row.id} className="bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3">
                  <Link href={`/yakit-kartlari/${row.id}`} className="block cursor-pointer">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-white font-mono font-semibold text-sm">{row.kart_no}</span>
                      <span className="text-zinc-500 text-xs">{FIRMA_LABELS[row.firma] || row.firma}</span>
                      {row.plate && <span className="text-zinc-400 font-mono text-xs">{row.plate}</span>}
                      {row.isleten_unvan && <span className="text-zinc-500 text-xs">· {row.isleten_unvan}</span>}
                      <span className={`text-[10px] px-1.5 py-0.5 rounded border ml-auto ${
                        row.is_active ? "bg-emerald-950 border-emerald-800 text-emerald-300" : "bg-zinc-800 border-zinc-700 text-zinc-500"
                      }`}>
                        {row.is_active ? "Aktif" : "Pasif"}
                      </span>
                    </div>
                    <p className="text-zinc-600 text-xs mt-1">{row.dolum_sayisi} dolum kaydı</p>
                  </Link>
                  {canCreate && (
                    <div className="flex gap-2 mt-2">
                      <button onClick={() => openEdit(row)}
                        className="text-xs px-3 py-1.5 rounded-lg border border-zinc-700 text-zinc-400 hover:text-white hover:border-zinc-600 transition-colors">
                        Düzenle
                      </button>
                      <button onClick={() => toggleActive(row)}
                        className="text-xs px-3 py-1.5 rounded-lg border border-zinc-700 text-zinc-400 hover:text-white hover:border-zinc-600 transition-colors">
                        {row.is_active ? "Pasife Al" : "Aktifleştir"}
                      </button>
                    </div>
                  )}
                </div>
              ))}
            </div>
          )}
        </div>
      </div>

      {showForm && (
        <div className="fixed inset-0 z-50 flex">
          <div className="flex-1 bg-black/60" onClick={() => setShowForm(false)} />
          <div className="w-full max-w-md bg-zinc-900 border-l border-zinc-800 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800 shrink-0">
              <h2 className="text-white font-semibold">{editing ? "Kartı Düzenle" : "Yeni Yakıt Kartı"}</h2>
              <button onClick={() => setShowForm(false)} className="text-zinc-500 hover:text-white text-xl leading-none">×</button>
            </div>
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              {saveError && <div className="bg-red-950 border border-red-800 text-red-300 text-sm px-3 py-2 rounded-lg">{saveError}</div>}
              <div className="grid grid-cols-2 gap-3">
                <label className="block">
                  <span className="text-zinc-400 text-xs font-medium mb-1 block">Kart No *</span>
                  <input value={form.kart_no} onChange={e => setForm(f => ({ ...f, kart_no: e.target.value }))}
                    className="w-full bg-zinc-800 border border-zinc-700 text-white text-sm px-3 py-2 rounded-lg focus:outline-none focus:border-zinc-500" />
                </label>
                <label className="block">
                  <span className="text-zinc-400 text-xs font-medium mb-1 block">Firma</span>
                  <select value={form.firma} onChange={e => setForm(f => ({ ...f, firma: e.target.value }))}
                    className="w-full bg-zinc-800 border border-zinc-700 text-white text-sm px-3 py-2 rounded-lg focus:outline-none focus:border-zinc-500">
                    {FIRMA_OPTS.map(o => <option key={o} value={o}>{FIRMA_LABELS[o]}</option>)}
                  </select>
                </label>
              </div>
              <label className="block">
                <span className="text-zinc-400 text-xs font-medium mb-1 block">Araç</span>
                <ComboboxSearch options={vehicles.map((v: any) => ({ value: v.id, label: v.plate }))}
                  value={form.vehicle_id} onChange={v => setForm(f => ({ ...f, vehicle_id: v }))}
                  placeholder="Araç seç..." emptyLabel="— Araç Yok —" />
              </label>
              <label className="block">
                <span className="text-zinc-400 text-xs font-medium mb-1 block">İşleten</span>
                <ComboboxSearch options={isletenler.map((i: any) => ({ value: i.id, label: i.unvan }))}
                  value={form.isleten_id} onChange={v => setForm(f => ({ ...f, isleten_id: v }))}
                  placeholder="İşleten seç..." emptyLabel="— İşleten Yok —" />
              </label>
              <div className="grid grid-cols-2 gap-3">
                <label className="block">
                  <span className="text-zinc-400 text-xs font-medium mb-1 block">Limit Türü</span>
                  <select value={form.limit_turu} onChange={e => setForm(f => ({ ...f, limit_turu: e.target.value }))}
                    className="w-full bg-zinc-800 border border-zinc-700 text-white text-sm px-3 py-2 rounded-lg focus:outline-none focus:border-zinc-500">
                    <option value="sinirsiz">Sınırsız</option>
                    <option value="miktar">Miktar (litre)</option>
                    <option value="tutar">Tutar (₺)</option>
                  </select>
                </label>
                <label className="block">
                  <span className="text-zinc-400 text-xs font-medium mb-1 block">Limit Değeri</span>
                  <input type="number" value={form.limit_degeri} onChange={e => setForm(f => ({ ...f, limit_degeri: e.target.value }))}
                    disabled={form.limit_turu === "sinirsiz"}
                    className="w-full bg-zinc-800 border border-zinc-700 text-white text-sm px-3 py-2 rounded-lg focus:outline-none focus:border-zinc-500 disabled:opacity-40" />
                </label>
              </div>
              <label className="block">
                <span className="text-zinc-400 text-xs font-medium mb-1 block">Kart Tipi</span>
                <select value={form.kart_tipi} onChange={e => setForm(f => ({ ...f, kart_tipi: e.target.value }))}
                  className="w-full bg-zinc-800 border border-zinc-700 text-white text-sm px-3 py-2 rounded-lg focus:outline-none focus:border-zinc-500">
                  <option value="normal">Normal</option>
                  <option value="akaryakit_only">Sadece Akaryakıt</option>
                </select>
              </label>
              <label className="block">
                <span className="text-zinc-400 text-xs font-medium mb-1 block">Not</span>
                <textarea value={form.notlar} onChange={e => setForm(f => ({ ...f, notlar: e.target.value }))} rows={3}
                  className="w-full bg-zinc-800 border border-zinc-700 text-white text-sm px-3 py-2 rounded-lg focus:outline-none focus:border-zinc-500 resize-none" />
              </label>
            </div>
            <div className="px-5 py-4 border-t border-zinc-800 flex gap-3 shrink-0">
              <button onClick={() => setShowForm(false)} className="flex-1 bg-zinc-800 text-zinc-300 font-medium text-sm py-2.5 rounded-xl hover:bg-zinc-700 transition-colors">İptal</button>
              <button onClick={save} disabled={saving || !form.kart_no.trim()}
                className="flex-1 bg-white text-zinc-950 font-semibold text-sm py-2.5 rounded-xl hover:bg-zinc-200 disabled:opacity-50 transition-colors">
                {saving ? "Kaydediliyor..." : "Kaydet"}
              </button>
            </div>
          </div>
        </div>
      )}
    </>
  );
}
