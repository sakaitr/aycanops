...
const DURUM_LABELS: Record<string, string> = {
  planlanan: "Planlanan", aktif: "Aktif", tamamlandi: "Tamamlandı", iptal: "İptal",
};
const DURUM_BADGE_CLASS: Record<string, string> = {
  planlanan: "bg-zinc-800 text-zinc-300",
  aktif: "bg-emerald-950 text-emerald-400",
  tamamlandi: "bg-blue-950 text-blue-400",
...
export default function ProjePage() {
  ...
  async function save() {
    ...
    try {
      const url = editing ? `/api/finans/proje/${editing.id}` : "/api/finans/proje";
  ...
  const canCreate = hasPermission(user, "finans_proje:create");
      ...
      <div className="min-h-screen bg-zinc-950 pt-16">
        <div className="max-w-3xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-2xl font-bold text-white">Projeler / İş Emirleri</h1>
              <p className="text-zinc-500 text-sm mt-0.5">{rows.length} proje</p>
            </div>
            {canCreate && (
          ...
          {loading ? (
            ...
            <div className="text-center py-16 text-zinc-600">Henüz proje tanımlanmadı</div>
          ) : (
                ...
                return (
                  <div key={row.id} onClick={() => openEdit(row)} className="bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 cursor-pointer hover:border-zinc-700 transition-colors">
                    <div className="flex items-center gap-2 flex-wrap">
                      {row.kod && <span className="font-mono text-zinc-500 text-xs">{row.kod}</span>}
                      <span className="text-white font-semibold text-sm">{row.ad}</span>
                      <span className={`text-xs px-2 py-0.5 rounded-full ${DURUM_BADGE_CLASS[row.durum] || "bg-zinc-800 text-zinc-300"}`}>
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-md bg-zinc-900 border-l border-zinc-800 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800 shrink-0">
              <h2 className="text-white font-semibold">{editing ? "Projeyi Düzenle" : "Yeni Proje"}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              <label className="block">
                <span className="text-zinc-400 text-xs font-medium mb-1 block">Proje Kodu</span>
