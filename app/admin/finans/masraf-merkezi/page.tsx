...
const EMPTY_FORM = { ad: "", company_id: "" };

export default function MasrafMerkeziPage() {
  ...
  function openEdit(row: any) {
    setEditing(row);
    setForm({ ad: row.ad, company_id: row.company_id || "" });
  ...
  async function save() {
    ...
    try {
      const url = editing ? `/api/finans/masraf-merkezi/${editing.id}` : "/api/finans/masraf-merkezi";
      const method = editing ? "PUT" : "POST";
      const res = await fetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      const d = await res.json();
      if (!d.ok) { setSaveError(typeof d.error === "string" ? d.error : "Kayıt hatası"); return; }
      toast.success(editing ? "Masraf merkezi güncellendi" : "Masraf merkezi eklendi");
      setShowForm(false);
      load();
    } finally { setSaving(false); }
  }

  const canCreate = hasPermission(user, "finans_masraf_merkezi:create");
      ...
      <div className="min-h-screen bg-zinc-950 pt-16">
        <div className="max-w-3xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-2xl font-bold text-white">Masraf Merkezleri</h1>
              <p className="text-zinc-500 text-sm mt-0.5">{rows.length} masraf merkezi</p>
            </div>
            {canCreate && (
          ...
          {loading ? (
            ...
            <div className="text-center py-16 text-zinc-600">Henüz masraf merkezi tanımlanmadı</div>
          ) : (
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-md bg-zinc-900 border-l border-zinc-800 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800 shrink-0">
              <h2 className="text-white font-semibold">{editing ? "Masraf Merkezini Düzenle" : "Yeni Masraf Merkezi"}</h2>
