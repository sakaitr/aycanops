...
export default function OdemeYontemiPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const r = await fetch("/api/finans/odeme-yontemi");
  ...
  async function save() {
    ...
    try {
      const url = editing ? `/api/finans/odeme-yontemi/${editing.id}` : "/api/finans/odeme-yontemi";
      const method = editing ? "PUT" : "POST";
      const res = await fetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      const d = await res.json();
      if (!d.ok) { setSaveError(typeof d.error === "string" ? d.error : "Kayıt hatası"); return; }
      toast.success(editing ? "Ödeme yöntemi güncellendi" : "Ödeme yöntemi eklendi");
      setShowForm(false);
      load();
    } finally { setSaving(false); }
  }

  const canCreate = hasPermission(user, "finans_odeme_yontemi:create");
      ...
      <div className="min-h-screen bg-zinc-950 pt-16">
        <div className="max-w-3xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between gap-4 mb-6">
            <div>
              <h1 className="text-2xl font-bold text-white">Ödeme Yöntemleri</h1>
              <p className="text-zinc-500 text-sm mt-0.5">{rows.length} ödeme yöntemi</p>
            </div>
            {canCreate && (
          ...
          {loading ? (
            ...
            <div className="text-center py-16 text-zinc-600">Henüz ödeme yöntemi tanımlanmadı</div>
          ) : (
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-full max-w-md bg-zinc-900 border-l border-zinc-800 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800 shrink-0">
              <h2 className="text-white font-semibold">{editing ? "Ödeme Yöntemini Düzenle" : "Yeni Ödeme Yöntemi"}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              <label className="block">
                ...
                <input value={form.ad} onChange={e => setForm(f => ({ ...f, ad: e.target.value }))}
                  placeholder="örn. Nakit, Havale/EFT, Kredi Kartı, Çek"
